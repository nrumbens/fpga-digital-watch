...
// Extends v3 so that when finished editing seconds a full second is passes
// before the second increments.
...
module user_top_watch_v4 #(
  ...
  restartable_rate_generator #(
      ...
      // keeps timer at 0 when editing time (so there is a full cycle after
      // editing)
      .run (!(mode_enable[0])),
      .tick(seconds_tick)
  );
